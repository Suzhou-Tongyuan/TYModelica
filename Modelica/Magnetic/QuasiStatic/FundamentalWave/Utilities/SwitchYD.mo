...
model SwitchYD "Y-D-switch"
  ...
  Electrical.QuasiStatic.Polyphase.Ideal.IdealClosingSwitch idealCloser(
    final m=m, 
    Ron=fill(Ron, m), 
    Goff=fill(Goff, m)) 
  ...
  Electrical.QuasiStatic.Polyphase.Ideal.IdealOpeningSwitch idealOpener(
    final m=m, 
    Ron=fill(Ron, m), 
    Goff=fill(Goff, m)) 
...
equation
  ...
  connect(plug_sn, idealOpener.plug_p) 
    annotation (Line(points={{-60,-100},{-60,-40}}, color={85,170,255}));
