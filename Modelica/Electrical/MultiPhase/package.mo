...
<a href=\"modelica://Modelica.Electrical.MultiPhase.UsersGuide.PhaseOrientation\">User's guide</a> on symmetrical components and orientation.
    ...
    protected
      constant Integer nBase=numberOfSymmetricBaseSystems(m);
      constant Integer mBase=integer(m/nBase);
    ...
    algorithm
      ...
      for i in 2:nBase loop
        for k in 1:mBase loop
          ...
          for j in 1:mBase loop
            oM[(i - 1)*mBase + k, (i - 1)*mBase + j] := oM[k,j] + oBase[i];
          end for;
        end for;
      end for;
      annotation (Documentation(info="<html>
...
<a href=\"modelica://Modelica.Electrical.MultiPhase.UsersGuide.PhaseOrientation\">User's guide</a> on symmetrical components and orientation.
    ...
    protected
      ...
      constant Integer mBase=integer(m/nBase) "Number of phases of base system";
    algorithm
      if mBase == 1 then
        ind := fill(0, 0);
      elseif mBase == 2 then
        ...
        for k in 1:nBase loop
          ...
          for j in 1:(mBase-1) loop
            ind[(mBase - 1)*(k - 1) + j] := j + 1 + mBase*(k - 1);
          end for;
        end for;
      end if;
      annotation (Documentation(info="<html>
...
<dl>
  ...
  annotation (Protection(hideFromBrowser=true),Documentation(info="<html>
<p>
