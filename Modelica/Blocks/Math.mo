...
y = u1 / u2;
  ...
  block MultiVector "Scalar Multiplication of Vectors: y[n] = k*u[n]"
    parameter Integer n = 1 "length of vector";
    Modelica.Blocks.Interfaces.RealInput u[n] "Connector of Real Vectors input signal" annotation (Placement(
          transformation(extent={{-140,40},{-100,80}})));
    Modelica.Blocks.Interfaces.RealInput k "Connector of Gain" annotation (Placement(
...
<span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\">This blocks computes the output </span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"><strong>y[n]</strong></span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"> as Scalar Multiplication of Vectors </span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"><strong>u[n]:</strong></span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51); background-color: rgb(217, 217, 217);\"><strong>y[n]=k*u[n];</strong></span>
  ...
  model DivVector "Scalar Division of Vectors: y[n] = u[n]/k"
    parameter Integer n = 1 "length of vector";
    Modelica.Blocks.Interfaces.RealInput u[n] "Connector of Vectors input signal" annotation (Placement(
          transformation(extent={{-140,40},{-100,80}})));
    Modelica.Blocks.Interfaces.RealInput k "Connector of Gain" annotation (Placement(
          transformation(extent={{-140,-80},{-100,-40}})));
    Modelica.Blocks.Interfaces.RealOutput y[n] "Connector of Vectors output signal" annotation (Placement(
          transformation(extent={{100,-10},{120,10}})));
    equation
    y=u/k;
    annotation(Icon(coordinateSystem(extent={{-100,-100},{100,100}},
...
<span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\">This blocks computes the output </span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"><strong>y[n]</strong></span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"> as Scalar Division of Vectors </span><span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\"><strong>u[n]:</strong></span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51); background-color: rgb(217, 217, 217);\"><strong>y[n]=u[n]/k;</strong></span>
  ...
  model VectorAdd "Addition of Vectors: y[n] = k1*u1[n]+k2*u2[n]"
...
<span style=\"color: rgb(51, 51, 51); background-color: rgb(243, 243, 243);\">This blocks computes output y[n] as sum of the two input Vectors signals u1[n] and u2[n]:</span>
</p>
<p>
...
<img src=\"modelica://Modelica/Resources/Images/Math/tan.png\"
  ...
  block Cot "Output the cotangent of the input"
    extends Interfaces.SISO(u(unit = "rad"));

  equation
    if abs(Modelica.Math.sin(u)) < Modelica.Constants.eps then
      y = Modelica.Constants.inf;
    else
      y = Modelica.Math.cos(u) / Modelica.Math.sin(u);
    end if;
    annotation(
      Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}},
      preserveAspectRatio = true,
      grid = {2, 2}), graphics = {Polygon(origin = {-86, 0},
      lineColor = {192, 192, 192},
      fillColor = {192, 192, 192},
      fillPattern = FillPattern.Solid,
      points = {{0, 90}, {-8, 68}, {8, 68}, {0, 90}}), Line(origin = {-86, 0},
      points = {{0, -80}, {0, 68}},
      ...
      smooth = Smooth.Bezier), Line(origin = {0, 0},
      points = {{-90, 0}, {68, 0}},
      color = {192, 192, 192}), Polygon(origin = {0, 0},
      lineColor = {192, 192, 192},
      fillColor = {192, 192, 192},
      fillPattern = FillPattern.Solid,
      points = {{90, 0}, {68, 8}, {68, -8}, {90, 0}}), Text(origin = {54, 48},
      lineColor = {192, 192, 192},
      extent = {{-36, 24}, {36, -24}},
      textString = "cot",
...
<p>
This blocks computes the output <strong>y</strong>
as <strong>cot</strong> of the input <strong>u</strong>:
</p>
<blockquote><pre>
y = <strong>cot</strong>( u );
</pre></blockquote>
<p>
if u is a multiple of pi, y = cot(u) is +/- infinity).
</p>
</html>"      ));
  end Cot;
...
y = base <strong>^</strong> u;
  ...
  block Pow "Power function block"
    extends Modelica.Blocks.Interfaces.SISO;
    parameter Boolean use_base_in = false "Use Input exponent" annotation(Evaluate=true);
    parameter Real base = 0 "Exponent" annotation(Dialog(enable=not use_base_in),Evaluate=true);
    Modelica.Blocks.Interfaces.RealInput base_in if use_base_in "Input exponent" 
...
This blocks is a power function, the base is the independent variable u, the power is the dependent variable y, and the exponent is base:
</p>
<pre><code >y = u ^ base;
</code></pre><p>
<br>
...
calculations are indicated by <code>valid = true</code>.
  ...
  model FFT "Fast Fourier Transform"

    parameter Integer n = 1 "Length of the input signal";
    Modelica.Blocks.Interfaces.RealInput x[n] "Input signal" 
      annotation(Placement(transformation(origin = {-120, 0},
      extent = {{-20, -20}, {20, 20}})));
    Modelica.Blocks.Interfaces.RealOutput yr[n] "Real part of the output signal" 
      annotation(Placement(transformation(origin = {110, 50},
      extent = {{-10, -10}, {10, 10}})));
    Modelica.Blocks.Interfaces.RealOutput yi[n] "Imaginary part of the output signal" 
      annotation(Placement(transformation(origin = {110, -50},
      extent = {{-10, -10}, {10, 10}})));
    function FFT "Fast Fourier Transform"
      ...
      output Real yr[n];
      output Real yi[n];
    external "C" mfft(x, yr, yi, n) 
      annotation(
      Include = "#include \"mfft.h\"",
      IncludeDirectory = "modelica://Modelica/Resources/Include",
      Library = "libfftw3-3",
  ...
  equation
    ...
    annotation(defaultComponentName = "FFT",
      Documentation(info="<html>
<p>
This model defines a Fast Fourier Transform (FFT) model, which takes an input signal x[n] of length n and
produces the transformed real and imaginary parts as output.
</p>
</html>"      ),
      Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}},
      grid = {2, 2}), graphics = {Rectangle(origin = {0, 0},
      fillColor = {255, 255, 255},
      fillPattern = FillPattern.Solid,
      extent = {{-100, 100}, {100, -100}}), Text(origin = {4, -0.5},
      lineColor = {0, 0, 0},
      extent = {{-86, 69.5}, {86, -69.5}},
      textString = "FFT",
      textStyle = {TextStyle.None},
      textColor = {0, 0, 0},
      horizontalAlignment = LinePattern.None), Text(origin = {0, 122},
      lineColor = {0, 0, 255},
      extent = {{-150, 20}, {150, -20}},
      textString = "%name",
      textColor = {0, 0, 255})}));
...
so one can plot the result directly as frequency lines.
  ...
  block RealFFTWithOutput "Sampling and FFT of input u, with output of amplitudes and phases"
    extends Modelica.Blocks.Interfaces.DiscreteBlock(final samplePeriod = 1 / (2 * f_res * div(ns, 2)));
    parameter Units.SI.Frequency f_max "Maximum frequency of interest";
    parameter Units.SI.Frequency f_res "Frequency resolution";
    final parameter Integer ns = Modelica.Math.FastFourierTransform.realFFTsamplePoints(f_max, f_res, f_max_factor = 5) "Number of samples";
    final parameter Integer nf = max(1, min(integer(ceil(f_max / f_res)) + 1, div(ns, 2))) "Number of frequency points";
    parameter String resultFileName = "realFFT.mat" "Result file: f, y_amplitudes, y_phases";
    output Integer info(start = 0, fixed = true) "Information flag from FFT computation";
    Modelica.Blocks.Interfaces.RealInput u 
      annotation(Placement(transformation(origin = {-120, 0},
      extent = {{-20, -20}, {20, 20}}),
  ...
  protected
    ...
    Integer iTick(start = 0, fixed = true) "Sample ticks";
  public
    Interfaces.RealOutput y_amplitudes[nf](start = zeros(nf), each fixed = true) "FFT amplitudes" 
      annotation(Placement(transformation(origin = {110, 60},
      extent = {{-10, -10}, {10, 10}})));
    Interfaces.RealOutput y_phases[nf](start = zeros(nf), each fixed = true) "FFT phases" 
      annotation(Placement(transformation(origin = {110, -50},
      extent = {{-10, -10}, {10, 10}})));
  algorithm
    when {sampleTrigger} then
    ...
    when terminal() then
      if iTick < ns then
        ...
        (info,y_amplitudes,y_phases) := Modelica.Math.FastFourierTransform.realFFT(buf, nf);
        assert(info == 0, "Error in FFT");
        Modelica.Math.FastFourierTransform.realFFTwriteToFile(startTime + (ns - 1) * samplePeriod, resultFileName, f_max, y_amplitudes, y_phases);
      end if;
    end when;
    annotation(Documentation(info = "<html>
<p>
